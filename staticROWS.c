...
int main(int argc, char* argv[]) {
    ...
    if (rank == 0) { // Master process
        ...
        for (int x = 0; x < WIDTH; x+=task_size) {
            ...
            MPI_Send(&x2, 1, MPI_INT, 1, 0, MPI_COMM_WORLD);

            //recieve result from slaves and add to 2d array
            int recived[task_size][HEIGHT];
            
           
            MPI_Recv(&recived, sizeof(int) * task_size * HEIGHT, MPI_BYTE, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        ...
        int x2 = x+task_size;
            ...
            MPI_Send(&x2, 1, MPI_INT, 2, 0, MPI_COMM_WORLD);

            //recieve result from slaves and add to 2d array
            int recived[task_size][HEIGHT];
            
           
            MPI_Recv(&recived, sizeof(int) * task_size * HEIGHT, MPI_BYTE, 2, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            // Wait for the task index to be received
            //MPI_Wait(&request, MPI_STATUS_IGNORE);
